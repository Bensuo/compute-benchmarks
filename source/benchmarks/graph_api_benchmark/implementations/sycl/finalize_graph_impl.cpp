...
#include "ext/oneapi/experimental/detail/properties/graph_properties.hpp"
...
static TestResult run([[maybe_unused]] const FinalizeGraphArguments &arguments, Statistics &statistics) {
    try {
        ...
        for (size_t iteration = 0; iteration < arguments.iterations; iteration++) {
            auto LocalGraph = arguments.firstFinalizeOnly ? graph_helpers::constructGraph(graphStructure, queue.get_context(), queue.get_device()) : graph;
            timer.measureStart();
            // Finalize the graph
